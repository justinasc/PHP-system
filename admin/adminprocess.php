...
class AdminProcess {
    ...
    function AdminProcess() {
		...
		/* Admin submitted delete entry form */ else if (isset($_POST['subdelentry'])) {
            $this->procDeleteEntry();
        }
		/* Admin submitted edit entry form */ else if (isset($_POST['subedit'])) {
	...
	/**
     * procUpdateEntry - If the submitted username is correct,
     * updating all details of entry.
     */
	 function procUpdateEntry(){
		 ...
		 $virsutinisRubas = $_POST['virsutinisRubas'];
		 $kiekis = $_POST['kiekis'];
		 
		 $spalva = "";
		 foreach($_POST['spalva'] as $spalva0) {
			 $spalva .= $spalva0 . " ";
		 }
		 ...
		 $q = "UPDATE " . TBL_DRABUZIAI . " SET pavadinimas = '$pavadinimas', virsutinis_rubas = '$virsutinisRubas', kiekis = '$kiekis',  spalva = '$spalva', dydis = '$dydis' WHERE id = '$id'";
		 $database->query($q);
	...
	function procDeleteEntry() {
		...
		$q = "DELETE FROM " . TBL_DRABUZIAI . " WHERE id = '$entryid'";
		$database->query($q);
		header("Location: " . $session->referrer);
	}

...
class Process {
    ...
    function procNewEntry() {
		...
		$spalva = "";
		foreach ($_POST["spalva"] as $spa) {
			$spalva .= $spa. " ";
		}
		
		/* Movie registration attempt */
	$retval = $session->newEntry($_POST['pavadinimas'] , $_POST['virsutinisRubas'] , $_POST['kiekis'] , $spalva , $_POST['dydis']);
	
		/* Clothes registration successful */
	if ($retval == 0) {
		$_SESSION['regsuccess'] = true;
		header("Location: " . $session->referrer);
		}

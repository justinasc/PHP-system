...
class Session {
     ...
     */
	 function newEntry($name, $virsutinisRubas, $kiekis, $spalva, $dydis) {
		 global $database, $form; 
		 //The database and form object
		 
		 $field = "pavadinimas";                                              //Use field name for movie, cia jei teksta ivesti ir patikrint ar vartotojas ivede teksta
		 if (!$name || strlen($name = trim($name)) == 0) {
			 $form->setError($field, "* Drabužio pavadinimas neįvestas<br>");
		 }
		...
		/* No errors, add the new clothe to the db*/
		if ($database->addNewEntry($this->username, $name, $virsutinisRubas, $kiekis, $spalva, $dydis)) {
			return 0; //New clothes added succesfully
		} else {
	 ...
	 function getEntries(){
		...
		foreach($database->getAllEntries() as $ses){
			echo $ses;
		}
